...
setwd("~/Dropbox/DP5_N-Bridge/Figure_Calculations/GSR/N40_GBC_mVWMWB1d/parcel_results/association_sensory")

# read in mask
mask <- read.table("network_mask.txt", header = TRUE)

# read in df
df <- read.table("Z_mean_Ket-Pla_HCS40_hpss_res-mVWMWB1d__CAB-NP_P718_BSNIP_parcels_gbc_mFz_pscalar.txt", header = FALSE)
...
pdf("mean_assoc_sens.pdf", width=2,height=4.5)
