...
setwd("~/Dropbox/DP5_N-Bridge/Figure_Calculations/GSR/Behaviour/PANSS_N")

# read in mask
mask <- read.table("network_mask.txt", header = TRUE)

# read in df
df <- read.table("panss_n_5000_dat_ztstat_pscalar.txt", header = FALSE)
...
pdf("panss_n_cortical.pdf", width=4.5,height=4.5)
ggplot(df_cortical, aes(df_cortical, x=variable, y=value, fill=variable, width=0.85)) + 
  ...
  coord_cartesian(ylim = c(-1.3, 1.4)) 
dev.off()
...
pdf("panss_n_subcortical.pdf", width=2.3,height=4.5)
...
pdf("panss_n_assoc_sens.pdf", width=2,height=4.5)
