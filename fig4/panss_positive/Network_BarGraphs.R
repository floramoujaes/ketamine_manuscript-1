...
setwd("~/Dropbox/DP5_N-Bridge/Figure_Calculations/GSR/Behaviour/PANSS_P")

# read in mask
mask <- read.table("network_mask.txt", header = TRUE)

# read in df
df <- read.table("panss_p_5000_dat_ztstat_pscalar.txt", header = FALSE)
...
pdf("panss_p_cortical.pdf", width=4.5,height=4.5)
...
pdf("panss_p_subcortical.pdf", width=2.3,height=4.5)
...
pdf("panss_p_assoc_sens.pdf", width=2,height=4.5)
